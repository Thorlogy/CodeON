...
public class RcxCompilerWorker implements ICompilerWorker {
    ...
    private Pair<Key, String> runBuild(Project project) {
        ...
        if ( result.getFirst() ) {
            // Die kompilierte .rcx-Binaerdatei base64-kodiert in die Server-Antwort legen.
            // Das Frontend (RcxConnection) reicht sie an die lokale RCX-Bridge weiter,
            // die sie per `nqc -Susb -d` ueber den Infrarot-Tower auf den RCX spielt.
            String base64Rcx = Util.getBase64EncodedBinary(targetFile);
            if ( base64Rcx != null ) {
                project.setCompiledHex(base64Rcx);
            } else {
                LOG.error("compiled .rcx could not be read from {}", targetFile);
