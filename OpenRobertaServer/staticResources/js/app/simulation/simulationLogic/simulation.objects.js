...
define(["require", "exports", "jquery", "util.roberta", "simulation.roberta", "simulation.math"], function (require, exports, $, UTIL, simulation_roberta_1, SIMATH) {
    ...
    var BaseSimulationObject = /** @class */ (function () {
        ...
        Object.defineProperty(BaseSimulationObject.prototype, "movable", {
            get: function () {
    ...
    var CircleSimulationObject = /** @class */ (function (_super) {
        ...
        function CircleSimulationObject(myId, myScene, mySelectionListener, type, p, maxSize, optColor) {
            ...
            _this.corners = [];
            _this.collisions = [];
            _this.lastMoveBumped = false;
            _this.lastMoveBumpedOld = false;
            _this.observers = [];
            _this.inEvacuationZone = false;
            _this.x = p.x;
            _this.y = p.y;
            if (maxSize) {
                _this.r = maxSize / 18;
        ...
        CircleSimulationObject.prototype.getTolerance = function () {
            return 1.01;
        };
        CircleSimulationObject.prototype.moveObstacleTo = function (dist) {
            if (!this.movable) {
                return false;
            }
        ...
        CircleSimulationObject.prototype.updateSensor = function (uCtx, obstacleList, collisionList) {
            if (!this.movable) {
                return;
            }
            this.lastMoveBumpedOld = this.lastMoveBumped;
            this.lastMoveBumped = false;
            var locationColor = uCtx.getImageData(this.x, this.y, 1, 1).data;
            this.inEvacuationZone = locationColor.toString() === [0, 0, 0, 255].toString();
            var thisCircle = { x: this.x, y: this.y, r: this.r * this.getTolerance() };
            var ground = obstacleList.slice(-1)[0]; // ground is always the last element in the personal obstacle list
            var interP = SIMATH.getMiddleIntersectionCircleRect(thisCircle, ground, SIMATH.getLinesFromRect(ground));
            if (interP) {
                var distP = SIMATH.getDistanceToCircle(interP, this);
                this.collisions.push(distP);
                this.lastMoveBumped = true;
            }
            for (var i = 0; i < obstacleList.length - 1; i++) {
                var myObstacle = obstacleList[i];
                if (myObstacle instanceof RectangleSimulationObject) {
                    var interP_1 = SIMATH.getMiddleIntersectionCircleRect(thisCircle, myObstacle, myObstacle.getLines());
                    if (interP_1) {
                        var distP = SIMATH.getDistanceToCircle(interP_1, this);
                ...
                if (myObstacle instanceof CircleSimulationObject) {
                    ...
                    var interPoint = SIMATH.getMiddleIntersectionPointCircles(thisCircle, myObstacle);
                    if (interPoint) {
                        var distP = SIMATH.getDistanceToCircle(interPoint, myObstacle);
        ...
        CircleSimulationObject.prototype.updateAction = function () {
            var diffP = { x: 0, y: 0 };
            this.collisions.forEach(function (diff) {
                diffP.x += diff.x;
                diffP.y += diff.y;
            });
            if (this.collisions.length > 0) {
                var dotProduct = SIMATH.getDotProduct(diffP, this.lastMove);
        ...
        CircleSimulationObject.prototype.addObserver = function (observer) {
            this.observers.push(observer);
        };
        CircleSimulationObject.prototype.notifyObservers = function () {
            for (var _i = 0, _a = this.observers; _i < _a.length; _i++) {
                var observer = _a[_i];
                observer.update(this);
            }
        };
        CircleSimulationObject.prototype.removeObserver = function (observer) {
    ...
    var RcjSimulationLabel = /** @class */ (function (_super) {
        __extends(RcjSimulationLabel, _super);
        function RcjSimulationLabel(myId, myScene, mySelectionListener, type, x, y, rcjType, index) {
            var _this = _super.call(this, myId, myScene, mySelectionListener, type, { x: x * 90 + 10, y: y * 90 + 10 }) || this;
            _this.w = 90;
            _this.h = 90;
            _this.type = type;
            _this.rcjType = rcjType;
            _this.index = String(index);
            switch (_this.rcjType) {
                case 'checkPoint':
                    _this.color = '#FFA500';
                    break;
                case 'start':
        ...
        RcjSimulationLabel.prototype.draw = function (ctx, uCtx) {
            ctx.save();
            if (this.rcjType) {
                ctx.lineWidth = 2;
        ...
        RcjSimulationLabel.prototype.handleMouseDown = function (e) {
            // nothing to do
        };
        RcjSimulationLabel.prototype.handleMouseMove = function (e) {
            // nothing to do
        };
        return RcjSimulationLabel;
    }(RectangleSimulationObject));
    exports.RcjSimulationLabel = RcjSimulationLabel;
});
