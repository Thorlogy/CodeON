(function () {
    ...
    function getSimulationInstance() {
        if (simModule && simModule.SimulationRoberta) return simModule.SimulationRoberta.Instance;
        if (window.require) {
            try {
                window.require(['simulation.roberta'], function (module) { simModule = module; });
            } catch (e) { return null; }
        }
        return null;
    }
    ...
    function buildRobot() {
        ...
        [-0.92, 0.92].forEach(function (x) {
            ...
            wheel.position.set(x, 0.28, 0.35);
    ...
    function init() {
        ...
        sun.castShadow = true;
        scene.add(sun);
        scene.add(new THREE.GridHelper(40, 20, 0x888888, 0xd0d0d0));
    ...
    function syncRobotPose() {
        ...
        robotMesh.position.z = (robot.pose.y - size.height / 2) * scale;
        robotMesh.rotation.y = -robot.pose.theta + Math.PI / 2;
        if (fieldMesh) fieldMesh.scale.set(Math.max(size.width / size.height, 1), Math.max(size.height / size.width, 1), 1);
    }
    ...
    function restore2dLayout() {
        var simScene = getSimulationScene();
        if (simScene && typeof simScene.centerBackground === 'function') simScene.centerBackground(false);
    }
    ...
    function toggle() { setMode(!enabled); }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        new MutationObserver(function () {
            if (enabled && !simDiv.classList.contains('rightActive')) setMode(false);
        }).observe(simDiv, { attributes: true, attributeFilter: ['class'] });
    });

    window.CodeOnSim3D = { init: init, resize: resize, setMode: setMode, toggle: toggle, isEnabled: function () { return enabled; } };
})();
