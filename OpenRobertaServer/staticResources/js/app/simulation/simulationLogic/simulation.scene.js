...
define(["require", "exports", "util.roberta", "jquery", "simulation.objects", "simulation.roberta", "robot.base", "robot.base.mobile", "robot.sensors"], function (require, exports, UTIL, $, simulation_objects_1, simulation_roberta_1, robot_base_1, robot_base_mobile_1, robot_sensors_1) {
    ...
    var RcjScoringTool = /** @class */ (function () {
        function RcjScoringTool(robot, configData, resetObstaclesCallback) {
            this.MAX_TIME = 8;
            this.POINTS_OBSTACLE = 20;
            this.POINTS_GAP = 10;
            this.POINTS_INTERSECTION = 10;
            this.POINTS_VICTIM_MULTI = 1.4;
            this.POINTS_DEADONLY_VICTIM_MULTI = 1.25;
            ...
            $('#rcjLoP')
                ...
                var x = Math.floor((rcj.robot.pose.x - 10) / 90);
                var y = Math.floor((rcj.robot.pose.y - 10) / 90);
        ...
        RcjScoringTool.prototype.handleShortCuts = function (event) {
            var code = '';
        ...
        RcjScoringTool.prototype.update = function (simObject) {
            ...
            if (simObject instanceof robot_base_mobile_1.RobotBaseMobile) {
                ...
                if (path == this.lastPath || path == this.lastPath + 1) {
                    ...
                    this.lastPath = path;
                    this.line = (function () {
                        for (var _i = 0, _a = _this.lineSensors; _i < _a.length; _i++) {
                            var sensor = _a[_i];
                    ...
                    if ((tile && tile.checkPoint) || path == 0) {
                        if (this.lastCheckPoint != tile) {
                            ...
                            var lastCheckPointIndex = this.lastCheckPoint['index'][0];
                ...
                var factor = Math.max(this.POINTS_VICTIM_MULTI - 0.05 * this.loPCounter, 1.25);
        ...
        RcjScoringTool.prototype.setRobot = function (robot) {
            this.robot = robot;
            this.initialPose = this.robot.initialPose;
            this.lineSensors = [];
            for (var key in robot) {
                if (robot.hasOwnProperty(key)) {
                    if (robot[key] instanceof robot_sensors_1.ColorSensorHex) {
