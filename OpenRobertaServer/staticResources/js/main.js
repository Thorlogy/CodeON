...
// ANTIGRAVITY PATCH: ENABLED PROXY FOR SVGS ONLY (Fixes Drag AND Click)
(function () {
    // Only apply to the SVG workspace (blocks) and FLYOUT, NOT the HTML toolbox (categories)
    // Expanded selector to ensure we catch the flyout even if it doesn't have .blocklySvg class
    // Expanded selector to catch V10/V9/V4 mix. Added .blocklyDraggable, .blocklyBlock, and raw svg tag.
    var blocklySelector = '.blocklySvg, .blocklyFlyout, .blocklyBlockCanvas, .blocklyDraggable, .blocklyBlock, svg';
    // ANTIGRAVITY PATCH: PROXY ENABLED (Fixes Drag AND Click)
    ['pointerdown', 'pointermove', 'pointerup', 'pointercancel'].forEach(function (type) {
        document.addEventListener(
            ...
            function (e) {
                ...
                // Debug log to see if proxy is firing for drags
                if (type === 'pointerdown') {
                    console.log("ANTIGRAVITY PROXY: Converting " + type + " on " + e.target.tagName + "." + e.target.className);
                }
                ...
                var mouseEvent = new MouseEvent(type.replace('pointer', 'mouse'), {
                    ...
                    clientY: e.clientY,
                    button: e.button,
                    buttons: e.buttons || 1, // ANTIGRAVITY FIX: Force buttons: 1 (Left Click) if missing/0
                    ctrlKey: e.ctrlKey,
                    shiftKey: e.shiftKey,
                    altKey: e.altKey,
                    metaKey: e.metaKey,
                });
                // ANTIGRAVITY FIX: preventDefault IS required to stop native emulation (double events)
                e.preventDefault();
    ...
    console.log('✓ ANTIGRAVITY V6: Proxy updated (WITH preventDefault, Buttons:1, Flyout support)');
    // console.log('ANTIGRAVITY: Proxy DISABLED for testing');
})();

// console.log('ANTIGRAVITY: Pointer-to-Mouse Proxy DISABLED');
...
require.config({
    ...
    paths: {
        ...
        'pythonLibrary.controller': 'js/app/roberta/controller/pythonLibrary.controller',
        'python_templates': 'js/app/roberta/controller/python_templates',
    },
    shim: {
        webots: {
...
require([
    ...
    console.log("ANTIGRAVITY DEBUG: Blockly required in main.js");
    if (Blockly) {
        console.log("ANTIGRAVITY DEBUG: Blockly object exists");
        if (Blockly.Msg) {
            console.log("ANTIGRAVITY DEBUG: Blockly.Msg exists with keys: " + Object.keys(Blockly.Msg).length);
        } else {
            console.log("ANTIGRAVITY DEBUG: Blockly.Msg is MISSING");
        }
    } else {
        console.log("ANTIGRAVITY DEBUG: Blockly object is MISSING");
    }
    // Fix for missing keys in early initialization
...
function init() {
    ...
    // ANTIGRAVITY PATCH: Force hide cover after 3 seconds just in case init hangs
    setTimeout(function () {
        if ($('.cover').is(':visible')) {
            console.log("ANTIGRAVITY: Force hiding cover");
            $('.cover').fadeOut(100);
    ...
    $.when(languageController.init())
        ...
        .then(function () {
            ...
            try { creatorsTheme.init(); } catch (e) { console.error("Theme init failed", e); }
            try { aiHelperController.init(); } catch (e) { console.error("AI Helper init failed", e); }
        ...
        .then(function () {
            ...
            console.log("ANTIGRAVITY: Init sequence completed, hiding cover normally.");
            $('.cover').remove(); // ANTIGRAVITY: Force remove instead of fadeOut
            $('body>.pace').remove(); // ANTIGRAVITY: Force remove

            // ANTIGRAVITY PATCH: Click Spy (Capture Phase)
            // Using true as 3rd arg to catch event before it reaches target/bubbling
            document.addEventListener('click', function (e) {
                var target = e.target;
                var tag = target.tagName;
                var id = target.id || '';
                var cls = target.className || '';
                console.log("ANTIGRAVITY CAPTURE SPY: Clicked on <" + tag + "> id='" + id + "' class='" + cls + "'");

                // Also check what element is actually top-most at this point
                // var topEl = document.elementFromPoint(e.clientX, e.clientY);
                // console.log("ANTIGRAVITY TOP ELEMENT: " + topEl.tagName + "." + topEl.className);
            }, true);

            // ANTIGRAVITY PATCH: Trigger resize to fix layout glitch
            setTimeout(function () {
                console.log("ANTIGRAVITY: Triggering window resize...");
...
function initProgramming(robot, extensions, opt_callback, opt_params) {
    ...
    if (!mainCallbackCalled) {
        ...
        $.when(robotController.init(robot, extensions)).then(function () {
            ...
            // ANTIGRAVITY PATCH: Force click handling for Toolbox Categories
            // Sometimes the standard Blockly click events are intercepted or blocked.
            // This global handler manually triggers the tree click logic.
            setTimeout(function () {
                console.log("ANTIGRAVITY: Installing Toolbox Click Handler");
                $(document).on('click', '.blocklyTreeRow', function (e) {
                    console.log("ANTIGRAVITY: Toolbox Category Clicked!");
                    // Blockly normally handles this internally. If it's blocked, we might need to manually trigger.
                    // But first, let's see if this log fires. If it does, the event reaches us.
                    // The issue might be that Blockly listens on 'mousedown' or 'pointerdown', not 'click'.

                    // Try to locate the component and trigger Blockly's internal handler if exposed?
                    // Unlikely to be easy.
                    // Alternative: Re-trigger a mousedown event if click is what we caught?
                    // Or, simply ensure pointer-events are auto on all children.
                    $('.blocklyTreeRow').css('pointer-events', 'auto');
                    $('.blocklyTreeRow *').css('pointer-events', 'auto');
                });

                // Also force pointer-events: auto on the main svg
                $('.blocklySvg').css('pointer-events', 'auto');
                $('.blocklyWidgetDiv').css('pointer-events', 'auto');
                $('.blocklyToolboxDiv').css('pointer-events', 'auto');
