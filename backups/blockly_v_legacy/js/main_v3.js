// ANTIGRAVITY UPGRADE: Blockly V10 Integrated. Proxy removed (Native Pointer Events supported).
window.onerror = function (msg, url, line, col, error) {
    console.error("ANTIGRAVITY GLOBAL ERROR:", msg, "\nLocation:", url, ":", line, ":", col, "\nStack:", error && error.stack);
};
require.config({
    ...
    paths: {
        ...
        blocks: '../blockly/blocks_compressed', // ANTIGRAVITY: Added separate blocks file
    ...
    shim: {
        ...
        blocks: {
            deps: ['blockly'], // ANTIGRAVITY: Blocks depend on core
        },
        confVisualization: {
            deps: ['blockly'],
        },
...
require([
    ...
    'bootstrap-table-locals',
    'blockly',
    'blocks', // ANTIGRAVITY: Load blocks definition
    'guiState.controller',
    'progList.controller',
    'logList.controller',
    'confList.controller',
...
function initProgramming(robot, extensions, opt_callback, opt_params) {
    ...
    if (!mainCallbackCalled) {
        ...
        $.when(robotController.init(robot, extensions)).then(function () {
            ...
            // We attach this immediately but delegating to document to ensure it works even if elements are re-created.
            console.log("ANTIGRAVITY: Installing Toolbox Click Handler (Delegated)");
            $(document).off('click', '.blocklyTreeRow'); // Remove old handlers if any
            $(document).on('click', '.blocklyTreeRow', function (e) {
                console.log("ANTIGRAVITY: Toolbox Category Clicked via Delegated Handler!", this);
                // Force pointer-events on children to ensure they don't block future clicks if that was the issue
                $(this).find('*').css('pointer-events', 'auto');

                // If Blockly is suppressing this, we might need to manually trigger the aria-expanded toggle or similar?
                // Visual inspection of the DOM might reveal aria-expanded="false" which needs to become "true".
                var ariaExpanded = $(this).attr('aria-expanded');
                console.log("ANTIGRAVITY: Current aria-expanded state:", ariaExpanded);

                // Note: The actual opening logic is internal to Blockly's Toolbox. 
                // If this event fires, it means we ARE receiving the click. 
                // If the drawer doesn't open, it's because Blockly's internal listener isn't firing or is bailing out.
                // We will try to simulate a 'mousedown' which blockly sometimes prefers.
            });
