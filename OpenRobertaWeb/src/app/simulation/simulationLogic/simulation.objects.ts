...
import { IObservableSimulationObject, IObserver, SimulationScene } from 'simulation.scene';
import * as SIMATH from 'simulation.math';

export interface IMovable {
    moveObstacleTo(dist: Point): boolean;

    updateSensor(uCtx: CanvasRenderingContext2D, obstacleList: ISimulationObstacle[], collisionList: ISimulationObstacle[]);
    updateAction();
}
...
export class CircleSimulationObject extends BaseSimulationObject implements IMovable, IObservableSimulationObject {
    x: number;
    y: number;
    r: number = 50;
    corners: Corner[] = [];
