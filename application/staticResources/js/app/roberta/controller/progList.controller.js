...
define(["require", "exports", "log", "util.roberta", "message", "progList.model", "userGroup.model", "program.model", "configuration.controller", "program.controller", "guiState.controller", "jquery", "table", "blockly", "bootstrap-table"], function (require, exports, LOG, UTIL, MSG, PROGLIST, USERGROUP, PROGRAM, CONFIGURATION_C, PROGRAM_C, GUISTATE_C, $, table_1, Blockly) {
    ...
    var view;
    var $progList;
    var $programNameTable;
    var $userGroupOptGroup;
    var $userGroupSelect;
    /**
    ...
    function initProgListEvents() {
        ...
        $programNameTable.onWrap('check-all.bs.table check.bs.table', function () {
        ...
        $programNameTable.onWrap('uncheck-all.bs.table', function () {
        ...
        $programNameTable.onWrap('uncheck.bs.table', function () {
        ...
        $('#deleteSomeProgHeader').onWrap('click', function () {
            ...
            var $confirmDeleteProgram = $('#confirmDeleteProgram');
    ...
    function showViewUserProgram() {
        ...
        var $selectProg = $("<select class='form-select filter' id='progListScopeSelect'>" +
        ...
        if (value.sharedWith && Object.keys(value.sharedWith).length > 1) {
            ...
            $.each(value.sharedWith, function (i, obj) {
                ...
                if (obj.type === 'User' && obj.label === 'Roberta') {
                    ...
                    result_1.push('<span class="typcn typcn-eye"></span>' + typeLabel + '&nbsp;<span class="value">' + obj.label + '</span>');
                }
                else if (obj.right === 'WRITE') {
                    result_1.push('<span class="typcn typcn-pencil"></span>' + typeLabel + '&nbsp;<span class="value">' + obj.label + '</span>');
                }
            });
            var resultString = '<div style="white-space:nowrap;"><span style="float:left;">';
            resultString += result_1[0];
            resultString +=
                '</span><a class="collapsed showRelations" href="#" style=" float:right;"' +
                    ' href="#" data-bs-toggle="collapse" data-bs-target=".relation' +
                    index +
                    '"></a></div>';
            for (var i = 1; i < result_1.length; i++) {
                resultString += '<div style="clear:both;" class="collapse relation' + index + '">';
                resultString += result_1[i];
                resultString += '</div>';
            }
            return resultString;
        }
    ...
    var titleActions = '<a href="#" id="deleteSomeProgHeader" class="deleteSomeProg disabled" rel="tooltip" lkey="Blockly.Msg.PROGLIST_DELETE_ALL_TOOLTIP" data-bs-original-title="' +
